...
def deal_repo_name_version(repo_name, dep_version):
    ...
    if r == 2 and repo_version:  # r == 2说明版本名不对
...
def deal_local_repo_dir(repo_id):
    ...
    tool_list = []
    vendor_list = []
    if not os.path.exists(file_name):  # 没有保存hero.txt文件
...
class Repo:
    ...
    def get_dep_tree_new(self):
        ...
        if db_data > 10 and up_list:  # 这个10什么意思(哈哈， 依赖树不完整)，上面函数返回的db_data不是 -1， 0， 1吗 uncertain
            ...
            for dep in repo_list:
                ...
                (r_db_data, r_up_list) = repoDep.get_dir_up_from_db(dep_repo.repo_name, dep_repo.v_name,
                                                                    dep_repo.v_hash)
                if r_db_data <= 0:  # 这里小于等于0不是只对没有上游依赖的依赖进行处理？ uncertain 可能由于数据库没保存，只能通过之前解析文件的方式获取
                    if self.mod_num <= 0:
                        (self.not_exit_list, self.all_dep_list,
                         all_dep_name_l) = dep_repo.get_dep_tree_nomod_new(layer, self.not_exit_list, self.all_dep_list,
                                                                           all_dep_name_l)
    ...
    def get_dm_local_new(self):
        ...
        if d_result != -1:
            ...
            if int(self.v_num) > 1:
                ...
                for p in self.mod_url:  # mod_url就是mod_list，保存了相对根目录的存在go.mod文件的路径
